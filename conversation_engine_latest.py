...
COMMANDS = {
    ...
    'who_owes_me': ['who owes me', 'my debtors', 'outstanding debts', 'debtors', 'who hasn\'t paid', 'debt list', 'owes me'],
...
class ConversationEngine:
    ...
    def _handle_idle(self, phone_number, text):
        ...
        # ---- Credit sale detection BEFORE AI categorizer ----
        # Phrases that clearly mean "customer owes me"
        credit_sale_signals = [
            'on credit', 'took goods', 'took items', 'collected goods',
            'owes me', 'owing me', 'credit sale', 'gave credit',
            'took on credit', 'collected on credit', 'sold on credit',
        ]
        credit_purchase_signals = [
            'i owe', 'i am owing', 'i bought on credit', 'credit purchase',
            'took on credit from', 'i dey owe', 'collected from on credit',
        ]
        is_credit_sale = any(sig in text_lower for sig in credit_sale_signals)
        is_credit_purchase = any(sig in text_lower for sig in credit_purchase_signals)
    ...
    def _handle_record_debt(self, phone_number, text):
        """Parse and record a credit sale (customer owes me)"""
        import re
        amount = self._extract_amount_from_text(text)
        if amount == 0:
        ...
        self.db.record_debt(phone_number, name, amount, 'owed_to_me', description=text)
        self.db.save_session(phone_number, STATE_IDLE, {})
    ...
    def _handle_record_i_owe(self, phone_number, text):
        """Parse and record a credit purchase (I owe supplier)"""
        amount = self._extract_amount_from_text(text)
        if amount == 0:
        ...
        self.db.record_debt(phone_number, name, amount, 'i_owe', description=text)
        self.db.save_session(phone_number, STATE_IDLE, {})
